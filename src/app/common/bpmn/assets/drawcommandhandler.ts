...
export class DrawCommandHandler extends go.CommandHandler {
  ...
  public alignColumn(): void {
    ...
    selectedParts.sort((a, b) => {
      if (a.actualBounds.y < b.actualBounds.y) {
        return -1;
      }
      if (a.actualBounds.y > b.location.y) {
        return 1;
      }
      return 0;
    });
    // sumatoria de todos los anchos de elementos
    let elementsWidth: number[] = [];
    for (let index = 0; index < selectedParts.length; index++) {
      elementsWidth.push(selectedParts[index].actualBounds.height);
    }
    // comienzo del eje x
    const startAxisX = selectedParts[0].actualBounds.y;
    // final del eje x
    const endAxisX =
      selectedParts[selectedParts.length - 1].actualBounds.y +
      selectedParts[selectedParts.length - 1].actualBounds.height;
    // ancho fijo del elemento
    const currentWidth =
      elementsWidth.reduce((previus, current) => previus + current) /
      selectedParts.length;
    // a la diferencia del eje x se le resta el numero de ancho fijos de los elementos seleccionados
    const subtotal =
      endAxisX - startAxisX - currentWidth * selectedParts.length;
    // el total se divide entre el numero de elementos que se quieren distribuir TODO:
    const total = subtotal / (selectedParts.length - 1);
    ...
    for (let i = 0; i < selectedParts.length - 1; i++) {
      ...
      const curBottomSideLoc =
        current.actualBounds.y + current.actualBounds.height + total;
  ...
  public alignRow(): void {
    ...
    selectedParts.sort((a, b) => {
      if (a.actualBounds.x < b.actualBounds.x) {
        return -1;
      }
      if (a.actualBounds.x > b.location.x) {
        return 1;
      }
      return 0;
    });
    ...
    let elementsWidth: number[] = [];
    for (let index = 0; index < selectedParts.length; index++) {
      elementsWidth.push(selectedParts[index].actualBounds.width);
    }
    // comienzo del eje x
    const startAxisX = selectedParts[0].actualBounds.x;
    // final del eje x
    const endAxisX =
      selectedParts[selectedParts.length - 1].actualBounds.x +
      selectedParts[selectedParts.length - 1].actualBounds.width;
    // ancho fijo del elemento
    const currentWidth =
      elementsWidth.reduce((previus, current) => previus + current) /
      selectedParts.length;
    ...
    for (let i = 0; i < selectedParts.length - 1; i++) {
      ...
      const curRightSideLoc =
        current.actualBounds.x + current.actualBounds.width + total;
      const next = selectedParts[i + 1];
