...
import { BPMNLinkingTool, BPMNRelinkingTool, BPMNPanningTool } from './assets/classes';
...
import { BpmnEventType, StartEventType, GatewayType, ActivityType, LanePosition, ActivityInstance } from './common/bpmn.enum';
import { CircleMenu } from './circleMenu/circleMenu';
import { headerNodeTemplate } from './templates/header.template';
import { relayoutDiagram, setSequenceLinkDefaultFlow, setSequenceLinkConditionalFlow, setSequenceLinkRegularFlow, verificarPosicion, isPool, setActivityType, clearActivityType, isUnoccupied, validConnectionLinksBetweenGroup, selectionChildren  } from './common/bpmn.functions';
...
import { startEventPalette, intermediateEventPalette, endEventPalette, mainPalette } from './palette/palette.seeds';
...
export class BpmnScript {
  ...
  public init(idDivDiagram: string = 'myDiagramDiv') {
    ...
    this.myDiagram = this.$(go.Diagram, idDivDiagram, {
      ...
      mouseDrop: () => {
        ...
        if(!isPool(this.myDiagram) || element.data.category == PaletteCategory.pool || element.data.category == PaletteCategory.header || element.data.category == PaletteCategory.group || element.data.category == PaletteCategory.annotation){
          const ok = this.myDiagram.commandHandler.addTopLevelParts(this.myDiagram.selection, true);
          if (!ok) {
            this.myDiagram.currentTool.doCancel();
          }
        }else if(element.data.group === null || element.data.group === undefined || !element.data.group){
      ...
      mouseDragOver: () => {
        const element = this.myDiagram.selection.first();
        if(isPool(this.myDiagram) && element && element.data.category != PaletteCategory.pool && element.data.category != PaletteCategory.header && element.data.category != PaletteCategory.group && element.data.category != PaletteCategory.annotation){
      ...
      "relinkingTool.fromHandleArchetype":
        this.$(go.Shape, "Diamond", { segmentIndex: 0, cursor: "pointer", desiredSize: new go.Size(8, 8), fill: this.bpmnConfig.LinksSymbolConnectedFill, stroke: this.bpmnConfig.LinksSymbolConnectedStroke }),
      "relinkingTool.toHandleArchetype":
        this.$(go.Shape, "Diamond", { segmentIndex: -1, cursor: "pointer", desiredSize: new go.Size(8, 8), fill: this.bpmnConfig.LinksSymbolConnectedFill, stroke: this.bpmnConfig.LinksSymbolConnectedStroke }),
      "linkReshapingTool.handleArchetype":
        this.$(go.Shape, "Diamond", { desiredSize: new go.Size(7, 7), fill: this.bpmnConfig.LinksCornerFill, stroke: this.bpmnConfig.LinksCornerStroke }),
    });

    const nodeTemplateMap = new go.Map<string, go.Node>();
    nodeTemplateMap.add(PaletteCategory.boundary, boundaryEventItemTemplate(go, this.$, this.bpmnConfig));
    nodeTemplateMap.add(PaletteCategory.event, eventNodeTemplate(go, this.$, this.bpmnConfig));
    nodeTemplateMap.add(PaletteCategory.gateway, gatewayNodeTemplate(go, this.$, this.bpmnConfig));
    nodeTemplateMap.add(PaletteCategory.annotation, annotationNodeTemplate(go, this.$, this.bpmnConfig));
    nodeTemplateMap.add(PaletteCategory.dataObject, dataObjectNodeTemplate(go, this.$, this.bpmnConfig));
    nodeTemplateMap.add(PaletteCategory.dataWarehousing, dataStoreNodeTemplate(go, this.$, this.bpmnConfig));
    nodeTemplateMap.add(PaletteCategory.group, groupNodeTemplate(go, this.$, this.bpmnConfig));
    nodeTemplateMap.add(PaletteCategory.header, headerNodeTemplate(go, this.$, this.bpmnConfig));
    nodeTemplateMap.add(PaletteCategory.privateProcess, privateProcessNodeTemplate(go, this.$, this.bpmnConfig));

    const groupTemplateMap = new go.Map<string, go.Group>();
    groupTemplateMap.add(PaletteCategory.activity, activityNodeTemplate(go, this.$, this.bpmnConfig));
    groupTemplateMap.add(PaletteCategory.subprocess, subProcessGroupTemplate(go, this.$, this.bpmnConfig));
    groupTemplateMap.add(PaletteCategory.lane, swimLanesGroupTemplate(go, this.$, this.myDiagram, this.bpmnConfig));
    groupTemplateMap.add(PaletteCategory.pool, poolGroupTemplate(go, this.$, this.bpmnConfig));

    // link template
    const linkTemplateMap = new go.Map<string, go.Link>();
    linkTemplateMap.add(PaletteCategory.msg, messageFlowLinkTemplate(go, this.$, this.bpmnConfig));
    linkTemplateMap.add(PaletteCategory.annotation, annotationAssociationLinkTemplate(go, this.$, this.bpmnConfig));
    linkTemplateMap.add(PaletteCategory.data, dataAssociationLinkTemplate(go, this.$, this.bpmnConfig));
    linkTemplateMap.add('', sequenceLinkTemplate(go, this.$, this.myDiagram, this.bpmnConfig));
    ...
    this.myDiagram.addDiagramListener('LinkDrawn', e => {
      ...
      } else if (e.subject.fromNode.category === PaletteCategory.dataObject || e.subject.toNode.category === PaletteCategory.dataObject) {
        e.subject.category = PaletteCategory.data;
      } else if (e.subject.fromNode.category === PaletteCategory.dataWarehousing || e.subject.toNode.category === PaletteCategory.dataWarehousing) {
        e.subject.category = PaletteCategory.data;
      }
    });
    ...
    this.circleMenu = new CircleMenu(go, this.$, this.myDiagram, this.bpmnConfig, null, this.eventBus);
    ...
    this.myDiagram.commandHandler.pasteSelection = () => {
      ...
      selections.each( (sel: any) => {
        ...
        if(!val.verif && val.node.fa.value != null && val.node.fa.value.category == PaletteCategory.lane){
  ...
  public setCircleMenuSeed(dataArray: CircleMenuDataArray[], type: string, position: string = '') {
    this.circleMenu.asignTranslateSeed(dataArray,type,position);
  }

  public setPalette(palette: PaletteSeed) {
    const paletteNew = new BpmnPalette(go, this.$, this.eventBus ? this.eventBus : null, this.bpmnConfig);
  ...
  private listenEvents() {
    this.eventBus.subscribe(BpmnEventType.selectContextMenuItem, (data: BpmnContextMenu) => {
      ...
      switch (data.type) {
        ...
        case BpmnContextMenuType.toRegularSequenceConnector:
          setSequenceLinkRegularFlow(<go.Link>this.myDiagram.selection.first(), this.myDiagram);
          break;
        case BpmnContextMenuType.toStandardSequenceConnector:
          setSequenceLinkDefaultFlow(<go.Link>this.myDiagram.selection.first(), this.myDiagram);
          break;
        case BpmnContextMenuType.toConditionalSequenceConnector:
          setSequenceLinkConditionalFlow(<go.Link>this.myDiagram.selection.first(), this.myDiagram);
          break;

        case BpmnContextMenuType.multipleInstancesTask:
          setActivityType(<go.Group>this.myDiagram.selection.first(), this.myDiagram, ActivityInstance.isParallel);
          break;
        case BpmnContextMenuType.loopIntancesTask:
          setActivityType(<go.Group>this.myDiagram.selection.first(), this.myDiagram, ActivityInstance.isLoop);
          break;
        case BpmnContextMenuType.simpleTask:
          clearActivityType(<go.Group>this.myDiagram.selection.first(), this.myDiagram);
          break;
        default:
          break;
      }
  ...
  private addAnnotation(): void {
    const circleMenu = new CircleMenu(go, this.$, this.myDiagram, this.bpmnConfig);
    const annotation = mainPalette(go, this.$).model.nodeDataArray.filter(item => item.category === PaletteCategory.annotation).shift();
    circleMenu.createNodeAndLink(annotation, this.myDiagram.selection.first());
  }

  private addLane(position: LanePosition): void {
    const lane = mainPalette(go, this.$).model.nodeDataArray.filter(item => item.category === PaletteCategory.lane).shift();
    ...
    const currentLane = <any>this.myDiagram.model.nodeDataArray.filter(item => item.category === PaletteCategory.lane).shift();
    ...
    if (isLane) {
      newnode.location = new go.Point(selection.location.x, position === LanePosition.up ? selection.location.y - 1 : selection.location.y + 1);
    } else {
      if (position === LanePosition.up) {
        newnode.location = new go.Point(selection.location.x, selection.location.y - 1);
      }
    }
  ...
  private addEdgeEvent(eventType: StartEventType) {
    const circleMenu = new CircleMenu(go, this.$, this.myDiagram, this.bpmnConfig);
    const newData = intermediateEventPalette(go, this.$).model.nodeDataArray.filter(item => item.eventType === eventType).shift();
    circleMenu.addEventBorderByNode(newData)
  }

  private convertToSubprocess() {
    const data = this.myDiagram.model.findNodeDataForKey(this.myDiagram.selection.first().data.key);
    if (data) {
  ...
  private convertEventTo(eventType: StartEventType) {
    ...
    newData = newData.filter(item => (item.category === category && item.eventType === eventType && item.eventDimension === eventDimension));
  ...
  initModel(){
    ...
    [
      { key: 555, category: PaletteCategory.pool, text: '', isGroup: true, loc: "0 0" },
      { key: 444, category: PaletteCategory.lane, text: '', isGroup: true, group: 555, size: '200 150', loc: "0 0" },
      { key: 333, category: PaletteCategory.event, text: '', group: 444, eventType: 1, eventDimension: 1, loc: '70 75' }
    ]);
    ...
    this.myDiagram.selection.each(node => {
      ...
      if (node.data && (node.data.category === 'activity' || node.data.category === 'subprocess')) {
        ...
        if (!node.data.boundaryEventArray) {
          this.myDiagram.model.setDataProperty(node.data, 'boundaryEventArray', []);
        }
  ...
  newDocument() {
    if (this.myDiagram.isModified) {
      const save = confirm('Would you like to save changes to ' + this.getCurrentFileName() + '?');
      if (save) {
        this.saveDocument();
      }
    }
  ...
  resetModel() {
    ...
    (this.myDiagram.model as go.GraphLinksModel).linkFromPortIdProperty = 'fromPort';
    (this.myDiagram.model as go.GraphLinksModel).linkToPortIdProperty = 'toPort';
