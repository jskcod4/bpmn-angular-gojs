...
export function circleMenuRightTemplate(go: any, $: any, circleMenu: CircleMenu, config: BpmnConfig) {
  ...
  let data = circleMenu.setAdornmentCircleMenu(rightSymbols.dataArray,circleMenu.myDiagram.selection.first().data);

  let template = $(go.Panel, "Vertical",
    { defaultStretch: go.GraphObject.Vertical ,alignment: go.Spot.Right, alignmentFocus: go.Spot.Left, 
      itemArray: data,
