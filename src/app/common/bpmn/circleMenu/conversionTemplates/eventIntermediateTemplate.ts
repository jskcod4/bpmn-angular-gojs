...
export function eventIntermediateTemplete(go: any, $: any, circleMenu: CircleMenu, config: BpmnConfig, Type: string = ''): go.Adornment{

  const palNodeTemplateMap = new go.Map();
  if(Type == 'boundary'){
    palNodeTemplateMap.add('event', eventButtonTemplate(go, $, circleMenu, 'boundary', config));
  }else{
  ...
  return $(go.Adornment, "Spot",
    ...
    $(go.Panel, "Horizontal",
      ...
      $(go.Panel, "Auto",
        ...
        $(go.Panel, "Horizontal",
          { defaultStretch: go.GraphObject.Vertical ,alignment: go.Spot.Bottom, alignmentFocus: go.Spot.Top, padding: 5,
            itemArray: circleMenu.eventIntermediateDataButton,
