...
import { SimbolosMenuCircular, CircleMenuDataArray, CircleMenuSeed } from './circleMenu.interface';
...
import { relayoutDiagram, positionEventBorder, verificarPosicion, validConnectionLinksBetweenGroup } from '../common/bpmn.functions';
...
export class CircleMenu {
  ...
  constructor(go: any, $: any, diagram: go.Diagram, config: BpmnConfig, bpmnScript?: BpmnScript, eventBus?: BpmnEventBus) {
    this.$ = $;
    this.go = go;
  ...
  init = (): void => {
    ...
    this.myDiagram.addDiagramListener('ChangedSelection', (diagramEvent) => {
      if(diagramEvent.diagram.selection.first()) console.log('cambio seleccion',diagramEvent.diagram.selection.first().data);
      ...
      if (element != null){
        ...
        if(val){
          ...
          if(this.nodeCircleMenu){
            if(this.nodeCircleMenu.adornedObject) this.nodeCircleMenu.adornedPart.removeAdornment("menuCircular");
          }
          this.nodeCircleMenu = circleMenuTemplate(this.go, this.$, this, this.config);
          const category = element.category;
          if (category == 'activity' || category == 'Pool' || category == 'Lane' || category == 'event' || category == 'subprocess' || category == 'gateway' || category == 'dataobject' || category == 'datastore' || category == 'privateProcess' || category == 'annotation' || category == 'boundary'){
  ...
  asignTranslateSeed = (dataArray: CircleMenuDataArray[] | CircleMenuSeed[], type: string, position: string) => {
    switch(type){
  ...
  setAdornmentCircleMenu = (data: any, simbol: any): any => {
    ...
    if(select.length > 0){
      return data.filter( item => {
        if(category == 'Pool' && (item.category == 'div2' || item.category == 'div3' || item.category == 'div')){
  ...
  setConversionSimbol = (e: any, button: any, $: any): void => {
    ...
    switch(category) {
      case 'activity':
        this.assignAdormentConversion(activityTemplete(this.go, $, this, this.config), node)
        break
      case 'event':
        this.assignAdormentConversion(eventStartTemplete(this.go, $, this, this.config), node)
        break;
      case 'eventIntermediate':
        this.assignAdormentConversion(eventIntermediateTemplete(this.go, $, this, this.config), node)
        break;
      case 'boundary':
        this.assignAdormentConversion(eventIntermediateTemplete(this.go, $, this, this.config, 'boundary'), node)
        break;
      case 'eventEnd':
        this.assignAdormentConversion(eventEndTemplete(this.go, $, this, this.config), node)
        break;
      case 'gateway':
        this.assignAdormentConversion(gatewayTemplete(this.go, $, this, this.config), node)
        break;
      case 'subprocess':
        this.assignAdormentConversion(subprocessTemplete(this.go, $, this, this.config), node)
        break;
      case 'eventBorder':
        this.assignAdormentConversion(eventBorderTemplete(this.go, $, this, this.config), node)
        break;
    }
  }
  ...
  setEventBorderSimbol = (button: any): void => {
    var node = button.part
    this.assignAdormentConversion(eventBorderTemplete(this.go, this.$, this, this.config), node)
  }
  ...
  convertirSimbol = (e: any, button: any): void => {
    ...
    var data = e.diagram.model.findNodeDataForKey(e.diagram.selection.first().data.key);
  ...
  addLane = (e: any, button: any, size?: any): void => {
    ...
    let lane = { category: PaletteCategory.lane, text: 'Process', isGroup: true, group: 0, size: element.data.size };
    ...
    if (button.data._data.position == 'top'){
      newnode.location = new this.go.Point(element.location.x, element.location.y - 1);
    } else if(button.data._data.position == 'bottom' && element.data.category == 'Lane'){
      newnode.location = new this.go.Point(element.location.x, element.location.y + 1);
    }
  ...
  swinDivLanes = (e: any, button: any): void => {
    ...
    if(group.length < button.data._data.quantity) {
      for (let index = 0; index < (button.data._data.quantity - group.length); index++) {
        if(group.length > 0) {
  ...
  createNodeAndLink = (data: any, fromnode: any) => {
    ...
    if(data.category == 'annotation'){
      let variable = this.myDiagram.selection.first().linksConnected.pb._dataArray[0];
  ...
  typeConversionSimbol = (simbol: any) => {
    ...
    switch(simbol.category){
      case "task":
        ...
        break;
      case "gateway":
        data = this.gatewayDataSeed.filter((item) => item.gatewayType == simbol.type);
        break;
      case "event":
        data = this.eventStartDataSeed.filter((item) => item.eventType == simbol.type);
        break;
      case "eventIntermediate":
        data = this.eventIntermediateDataSeed.filter((item) => item.eventType == simbol.type);
        break;
      case "eventEndDataSeed":
        data = this.eventEndDataSeed.filter((item) => item.eventType == simbol.type)
        break;
      case "subprocess":
        data = this.subprocessDataSeed.filter((item) => item.subprocessType == simbol.type)
        break;
      case "boundary":
        data = this.eventIntermediateDataSeed.filter((item) => item.eventType == simbol.type)
