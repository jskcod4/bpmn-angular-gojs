...
import { bpmnMenuContext, BpmnMenuType } from "./bpmn-menu.context";
...
export function init() {
  ...
  const openDocumentDiv = document.getElementById(
    "openDocument"
  ) as HTMLDivElement;
  openDocumentDiv.style.visibility = "hidden";
  // hides remove HTML Element
  const removeDocumentDiv = document.getElementById(
    "removeDocument"
  ) as HTMLDivElement;
  ...
  const tooltiptemplate = $<go.Adornment>(
    "ToolTip",
    $(
      go.TextBlock,
  ...
  const boundaryEventMenu = $<go.Adornment>( // context menu for each boundaryEvent on Activity node
    "ContextMenu",
    $(
      "ContextMenuButton",
      ...
      // in the click event handler, the obj.part is the Adornment; its adornedObject is the port
      {
        click: function (e: go.InputEvent, obj: go.GraphObject) {
          removeActivityNodeBoundaryEvent(
            (obj.part as go.Adornment).adornedObject
          );
        },
      }
    )
  ...
  const laneEventMenu = $<go.Adornment>( // context menu for each lane
    "ContextMenu",
    $(
      "ContextMenuButton",
          ...
          // in the click event handler, the obj.part is the Adornment; its adornedObject is the port
          {
            click: function (e: go.InputEvent, obj: go.GraphObject) {
              setSequenceLinkDefaultFlow(
                (obj.part as go.Adornment).adornedObject as go.Link
              );
            },
          }
        ),
...
export function newDocument() {
  ...
  if (myDiagram.isModified) {
    const save = confirm(
      "Would you like to save changes to " + getCurrentFileName() + "?"
    );
    if (save) {
      saveDocument();
    }
